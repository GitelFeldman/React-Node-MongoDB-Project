...
export const addUser = async (req, res) => {
    ...
    if (validate.error)
        return res.status(400).json({ type: "not valid body", message: validate.error.details[0] })
    try {
...
export const login = async (req, res) => {
    ...
    try {
        ...
        return res.status(404).json({ type: "no such user", message: "User not found. Please sign up." });
